...
| **玉衡** | yuheng | 自检——编码后对照契约自测 | 开发自测 | **必经**（天权/开阳驱动唤醒）|
...
| 天权/开阳→玉衡 | 代码（驱动唤醒，玉衡独立自检）| 玉衡自检（天权/开阳编码后@唤醒）|
...
任何星发现**契约外的问题**（改动影响白名单外 / 审查发现但不影响本次验收），不擅改，但必须登记到 `.taiyi/_workspace/interventions.md`。
...
每颗星遇到不该接的请求时，必须**明确拒绝**，不内耗、不硬接。拒绝不是"我不干了"，而是**"我不干了，但我告诉你该找谁、为什么"**——拒绝 = 拒绝 + 路由建议。每颗星拒绝时扮演自己范围的传声筒，把球精准传给下一站。

**三类拒绝触发（穷举）**：

| 类型 | 触发 | 例子 |
|------|------|------|
| **R1 输入不符** | 自审检查项不过（前置文件缺/字段缺/不可机检）| 天玑发现缺 design.md |
| **R2 错误@使用** | 请求不属于本星职责（语义错配）| @天权 做需求判断；@洞明 写代码 |
| **R3 能力边界** | 请求属于职责但超出能力（缺信息/需架构决策/熔断）| 天权遇到多方案抉择→BLOCKED |

**拒绝时标准三步动作（全星统一）**：

```
1. 明确拒绝 + 说清类型（R1/R2/R3）+ 说清原因（缺什么/错配在哪/卡在哪）
2. 更新 _workspace/status.md：
   - current_star_status = REJECTED（R1/R2）或 BLOCKED（R3）
   - block_reason = {类型 R1/R2/R3} + {原因} + {建议路由: 回哪颗星}
3. 给出路由建议（不自己执行）：
   - R1 缺前置 → 建议回上游补（查本星 SKILL 的"九星邻接表"）
   - R2 错误@ → 建议正确的星（查本星 SKILL 的"九星邻接表"）
   - R3 能力边界 → 建议升级用户/回天璇（架构决策）
```

> 路由建议的依据是各星 SKILL 自带的"九星邻接表"（自包含，不跨目录读）。邻接表是稳定锚（星职责不日常变），物理复制进每颗执行星，靠 test-references.sh 守一致性。

### 设计时验证假设（全星公共基线）

**纪律**：设计/路由决策前，先验证"方案依赖的前提是否成立"，不凭空设计。

任何星在做设计、拆解、路由判断前，凡涉及"我假设 X 成立"的地方，必须**先验证 X 真的成立**，再往下走。未验证的前提是隐患——基于错误前提的方案，执行得越完美偏离得越远。

**来源教训**（framework-refactor Epic 讨论中的三次凭空设计失误）：
1. INDEX.md 多分支并发冲突误判——未先查 git/worktree 实际开发模型就凭空设计
2. "环境跑不通"误判为"体系设计缺陷"——未先验证目标环境能力边界就归因
3. "查 CONTEXT.md 表路由"设计违反跨目录引用约束——未先验证 agent 注入机制读不读得到

**适用范围**：所有星。不只天璇（设计），天玑（拆解）、司衡（路由）、甚至天权（开工盘点）都适用。

**落地**：各星 SKILL 的偷懒借口对照表含此纪律的反驳条目（见各星 SKILL）。
...
| **太一文件** | 7.5 节：全局 `.taiyi/contracts/太一.md` | **不落地为单独文件**，契约分散在各 Task 契约中 | 设计稿 7.5 与 12.2 自相矛盾（全局 vs Epic 内）。采用 12.2 的 Epic 内契约，更合理 |
...
- ` + [char]96 + `GUIDE.md` + [char]96 + ` — 文档体系使用指南（目录结构+写权限矩阵+合并流程+三层职责链，开源工具说明书）
- `.taiyi/INDEX.md` — 项目顶层导航（洞明维护）
